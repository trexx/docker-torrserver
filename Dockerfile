...
RUN mkdir /tmp/src
RUN wget -qO- https://github.com/YouROK/TorrServer/archive/refs/tags/MatriX.141.7.tar.gz | tar --strip-components=1 -xzv -C /tmp/src
...
COPY --from=server --chmod=+x /tmp/src/server/torrserver /torrserver
